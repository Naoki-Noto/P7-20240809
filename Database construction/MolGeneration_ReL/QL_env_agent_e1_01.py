...
mols = 89
class QLAgent:
    def __init__(self, gamma=0.9, epsilon=1.0, alpha=0.7, epsilon_min=0.1, epsilon_decay=0.1, decay_interval=1000):
        self.gamma = gamma
    ...
    def end_episode(self):
        ...
        if self.episode_count % self.decay_interval == 0:
            new_epsilon = max(self.epsilon_min, self.epsilon - self.epsilon_decay)
            if new_epsilon != self.epsilon:
                self.epsilon = new_epsilon
                print(f"[Episode {self.episode_count}] Epsilon decayed to: {self.epsilon:.3f}")
