...
data = pd.read_csv('data_Human/data_Human.csv')
print(data)
duplicated_smiles = data[data.duplicated(subset='SMILES')]
print(duplicated_smiles)
duplicated_smiles.to_csv('data_Human_2/data_Human_dp.csv', index=False)

data_unique = data.drop_duplicates(subset='SMILES')
print(data_unique)
data_unique.to_csv('data_Human_2/data_Human_2.csv', index=False)
