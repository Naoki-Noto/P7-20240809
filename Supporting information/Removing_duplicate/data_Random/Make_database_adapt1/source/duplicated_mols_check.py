...
data = pd.read_csv('Human_made_smiles.csv')
print(data)
duplicated_smiles = data[data.duplicated(subset='SMILES')]
print(duplicated_smiles)
duplicated_smiles.to_csv('Human_made_smiles_dp.csv', index=False)

data_unique = data.drop_duplicates(subset='SMILES')
print(data_unique)
data_unique.to_csv('Human_made_smiles_2.csv', index=False)
...
print(f_smiles)
f_smiles.to_csv('Human_made_smiles_3.csv', index=False)
